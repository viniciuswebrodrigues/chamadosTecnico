...
$result_cont = "SELECT DATE_SUB(criado, INTERVAL 3 HOUR) AS criado, id, ide, acaminho, nivel, terminal, modelo, ultima_venda, sonda, atendimento
                FROM metroatm
...
foreach($result as $resultado_msg_cont) {
    echo '<tr>';
		...
		echo '<td <button type="button" class="button"></button>'."<a href='fit.php?id=".$resultado_msg_cont['id']."'>Responder</a>".'</td>';
