...
if ($SendCadCont) {
  ...
  VALUES(:id, :nome, :ide, :terminal, :modelo, :opcao, :data, :hora_inicio, :hora_termino, :falha_alegada, :alegada_outro, :falha_constatada, :constatada_outro, :acao_corretiva, :corretiva_outro, :conjunta, :atendimento, :pendencia, DATE_SUB(NOW(), INTERVAL 3 HOUR))";
  ...
  if ($result->execute()) {
    $_SESSION['msg'] = "<div class'alert alert-info alert-dismissible' role='alert'><button type='button' class='close' data-dismiss='alert'>&times</button><div class='alert-icon'><i class='icon-info'></i></div><div class='alert-message'><span>FIT enviada com sucesso!</span></div>";
    header("location: ../metro/resultcard.php");
  } else {
